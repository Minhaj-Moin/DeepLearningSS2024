...
class TestBatchNorm(unittest.TestCase):
    ...
    def test_forward(self):
        ...
        # print(mean,np.sum(np.square(mean - np.zeros(mean.shape[0]))), var)
    ...
    def test_reformat_image2vec(self):
        ...
        print(image_tensor.shape, vec_tensor.shape)
    ...
    def test_forward_test_phase(self):
        ...
        mean_input = np.mean(self.input_tensor, axis=0)
        var_input = np.var(self.input_tensor, axis=0)
        print(self.input_tensor.mean(axis=0))
        # print(np.square(mean + (mean_input/np.sqrt(var_input))))
        print("OutputMean",mean,"\nmean_input/np.sqrt(var_input)", mean + (mean_input/np.sqrt(var_input)),"\nsqr(mean+mean_input/sqrt(var_input))", np.square(mean + (mean_input/np.sqrt(var_input))))
    ...
    def test_forward_test_phase_convolutional(self):
        ...
        mean, var = TestBatchNorm._channel_moments(output, self.channels)
        mean_input, var_input = TestBatchNorm._channel_moments(self.input_tensor_conv, self.channels)
        print(mean, var, mean_input, var_input, np.square(mean + (mean_input / np.sqrt(var_input))))
